...
const TASKS = [
  ...
  // ── MOS-8 sub-deliverables (Scorecard Infrastructure) ────────────────────
  T('mos-8a', 'mos', 'Scorecard — Add Metric Row (admin modal)',
    '2026-06-15', ['RD'], ['BS', 'SN'], 'high',
    { status: 'done', subgroup: 'mos8-delivered',
      notes: 'Admin, exec, and dept heads can now add new metric rows directly inside any scorecard tab via a modal (name, owner, goal). No more SQL required.' }),
  T('mos-8b', 'mos', 'Scorecard — Archive Metric (restore section)',
    '2026-06-15', ['RD'], ['BS', 'SN'], 'med',
    { status: 'done', subgroup: 'mos8-delivered',
      notes: 'Each metric row has an archive button. Archived metrics collapse to a "Restore" section, keeping scorecards clean without permanent deletion.' }),
  T('mos-8c', 'mos', 'Scorecard — Real supervisor names in Owner dropdowns',
    '2026-06-15', ['RD'], ['BS', 'SN'], 'med',
    { status: 'done', subgroup: 'mos8-delivered',
      notes: 'Replaced all dummy placeholder names in the Owner dropdowns with the 18 actual BrightPath supervisors from the employee roster.' }),
  // ── MOS-8 sub-deliverables (Live Data Integrations) ──────────────────────
  T('mos-8d', 'mos', 'Zoho CRM → DAM Scorecard (OAuth 2.0 live integration)',
    '2026-06-15', ['RD'], ['BS', 'SN'], 'high',
    { status: 'done', subgroup: 'mos8-delivered',
      notes: 'Full OAuth 2.0 connection. Now auto-populates: New Referrals Received, Qualified Referrals, Active Referrals in Pipeline, UBS & Res Decision-to-Admission avg days.' }),
  T('mos-8e', 'mos', 'JazzHR → HR Scorecard (API key live integration)',
    '2026-06-15', ['RD'], ['BS', 'SN'], 'high',
    { status: 'done', subgroup: 'mos8-delivered',
      notes: 'API key connection. Now auto-populates: # Open Positions, # Open Direct Care, # Open BT, # Open IHS, # New Applications This Week, # Hires This Week.' }),
  T('mos-8f', 'mos', 'Therap → Residential / UBS / QAT (CSV export flow)',
    '2026-06-15', ['RD'], ['BS', 'SN'], 'high',
    { status: 'done', subgroup: 'mos8-delivered',
      notes: 'Documented the 4 exact CSV exports needed (GER Status, T-Log Completion, EVV Summary, Compliance/ISP) and the upload flow already built in Admin.' }),
  T('mos-8g', 'mos', 'Internal Trackers → HR & QAT (auto-sync)',
    '2026-06-15', ['RD'], ['BS', 'SN'], 'med',
    { status: 'done', subgroup: 'mos8-delivered',
      notes: 'Operations trackers (Onboarding, Training, Termination, UBS Staffing) now auto-push aggregated counts into the HR and QAT scorecards on every sync.' }),
  // ── MOS-8 sub-deliverables (Visibility) ──────────────────────────────────
  T('mos-8h', 'mos', 'Systems & Data Flow Map (Admin visibility page)',
    '2026-06-15', ['RD'], ['BS', 'SN'], 'med',
    { status: 'done', subgroup: 'mos8-delivered',
      notes: 'New page under Admin showing every data source BrightPath uses, connection type (Live API / CSV / Internal Sync / Manual), which scorecard departments each feeds, and a filterable metric-by-metric source table.' }),
